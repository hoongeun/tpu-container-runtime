/* copy from: https://github.com/ionosnetworks/edgetpu-rs */
...
pub fn main() {
    ...
    let interpreter = build_interpreter(model).expect("failed to build interpreter");

    let tensor_index = interpreter.inputs()[0];
    let required_shape = interpreter.tensor_info(tensor_index).unwrap().dims;
    let image = read_bmp(manifest_dir().join("data/resized_cat.bmp")).expect("faild to load image");
    if image.height != required_shape[1]
        || image.width != required_shape[2]
        || image.channels != required_shape[3]
    {
        eprintln!("Input size mismatches:");
        eprintln!("\twidth: {} vs {}", image.width, required_shape[0]);
        eprintln!("\theight: {} vs {}", image.height, required_shape[1]);
        eprintln!("\tchannels: {} vs {}", image.channels, required_shape[2]);
        return;
    }
    ...
    interpreter
        ...
        .unwrap()
        .copy_from_slice(image.data.as_ref());
...
fn build_interpreter(model: FlatBufferModel) -> Result<tflite::Interpreter, tflite::Error> {
